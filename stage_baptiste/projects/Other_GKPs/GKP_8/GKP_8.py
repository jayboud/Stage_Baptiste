...
if graph_wigner:
    ...
    plt.savefig(f"/Users/jeremie/Desktop/Stage_Baptiste/stage_baptiste/projects/Other_GKPs/GKP_8/figs/qubit_equiv_diagsup,j={j},d={d}")
...
if second_part:
    ...
    delta = 0.3
    dim = 100
    osc = (GKP(d,0,delta,dim).state+np.exp(1j*1*pi/5)*GKP(d,1,delta,dim).state+np.exp(1j*4*pi/5)*GKP(d,2,delta,dim).state
                        ...
                        +np.exp(1j*1*pi/5)*GKP(d,9,delta,dim).state).unit()
    ...
    n_op = a.dag()*a
    H = n**2
    rotlist = np.linspace(0,2*pi-np.arctan(n/m),5)
    tlist = np.linspace(0,pi/16,10)
    options = Options(store_states=True)  # get states even if e_ops are calculated
    rot = mesolve(n_op,osc,rotlist,[],[])
    # out = mesolve(H, osc, tlist, [], [])
